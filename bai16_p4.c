// input an array with n elements then, define longest ascending elements and print all of them on the screen
...
void show(int* arr, int startIndex, int length) {
	int i;
	printf("\nStart position: %d", startIndex, length);
	printf("\nThe length of this string: %d", length);
	printf("\nThe elements: ");
...
void theResult(int *arr, int n, int *start, int *end, int count)
{
	...
	if(count == n)
	{
		printf("Does not exist the longest ascending elements.");
	} else
	{
